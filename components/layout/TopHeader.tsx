  ...
  return (
    <div ref={ref} className="fixed top-4 right-6 z-50">
      ...
      <button
        onClick={() => setOpen(v => !v)}
        className={cn(
          'relative w-10 h-10 flex items-center justify-center rounded-full text-[#1267A4] bg-white border border-[#E5E7EB] shadow-sm hover:bg-[#EBF3FB] hover:border-[#1267A4]/30 transition-colors cursor-pointer',
          open && 'bg-[#EBF3FB] border-[#1267A4]/30'
        )}
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-12 w-[380px] bg-white rounded-2xl shadow-2xl border border-[#E5E7EB] overflow-hidden"
          >
            {/* En-tête */}
